...
def read_3band(img_id=None, train=True):
    ...
    predviduvanje
    :return vraka python recnik od sliki so nivniot img_id ili dokolku img_id ne e None togas vraka edna slika; Slikite se numpy.array() so
    dimenzii mxnx3:
    """
    ...
    if not img_id:
        if train:
                for file in os.listdir('three_band'):
                    id = file.strip('.tif')
                    if id in image_ids:
                        images[id] = (tiff.imread(os.path.join('three_band', file)).transpose([1, 2, 0]))
        else:
            for file in os.listdir('three_band'):
                id = file.strip('.tif')
